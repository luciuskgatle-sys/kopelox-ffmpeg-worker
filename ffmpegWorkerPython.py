...
async def choir_render_job(payload: dict):
    ...
    try:
        ...
        # Scale and pad each input to uniform size
        tile_width = 1920 // grid_cols
        tile_height = 1080 // grid_rows
        
        for idx, video in enumerate(video_files):
            offset = video['offset']
            # Trim video to start at offset, scale to tile size
            filter_parts.append(
        ...
        # Combine audio (mix all tracks)
        audio_filter = f"amix=inputs={num_videos}:duration=longest[outa]"
        ...
        ffmpeg_cmd.extend([
            ...
            '-c:a', 'aac',
            '-b:a', '192k',
            '-t', '60',  # Limit to 60 seconds for safety
            output_path
        ])
